"""Private PostgreSQL persistence for ingestion-job coordination and finalization."""
...
class PostgresIngestionJobCoordinationStore:
    def __init__(
        self,
        session_factory: sessionmaker,
        database_now: Callable[[Session], datetime],
        lifecycle_store: PostgresObjectLifecycleStore,
        enqueue_terminal_cleanup: Callable[..., None] | None = None,
    ) -> None:
        self._session_factory = session_factory
        self._database_now = database_now
        self._lifecycle_store = lifecycle_store
        self._enqueue_terminal_cleanup = (
            enqueue_terminal_cleanup
            or lifecycle_store.enqueue_terminal_cleanup_in_transaction
        )
        ...
        if terminal_decision is not None:
            values.extend(
                (
                    ...
                    PostgresIngestionJobCoordinationStore._terminal_policy_result(terminal_decision),
                )
            )
        return "\n".join(values)
    ...
    def _owns_current_unexpired_attempt(
        ...
        return PostgresIngestionJobCoordinationStore._owns_current_unexpired_token(
