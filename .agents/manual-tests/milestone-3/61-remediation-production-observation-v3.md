...
- Guide revision: `m3-remediation-61-v3`
- Supersedes: `m3-remediation-61-v2` (v1 and v2 are immutable; this revision is draft and
  unlocked)
...
### TC-61-06 — Authority-bound decision/reason contract and precedence gap
...
4. Exercise a candidate for which more than one exclusion condition is true and capture the
   serialized output without assigning a local precedence rule.
...
- Unknown values and incompatible decision/reason pairs fail closed and cannot be serialized as an
  accepted candidate decision.
- No current authoritative artifact supplies a precedence entry or JSON path for competing
  exclusion conditions. Therefore this guide does not assert that overlap, chunk count or token
  budget wins. The competing-condition sub-check is `BLOCKED: authority missing` until an approved
  authority defines the precedence; implementation order is not authority.

Evidence: the pinned authority entries/JSON pointers, serialized candidate decision list, exact
enum/reason assertions, unknown-value failure results, and an authority-gap record for competing
conditions.
...
no-contribution omission or unknown taxonomy value accepted is `FAILED`. An unresolved authority
gap for decision/reason mapping or competing-condition precedence is `BLOCKED`, not `PASSED`. An
