...
- Supersedes: unapproved `m3-remediation-issue-63-v5`; v5, v4, v3, v2 and v1 remain unchanged and must not be executed
- Guide revision: `m3-remediation-issue-63-v6`
- Approval status: approved and locked; claim-rule authority `m3-improvement-claim-v1` is `APPROVED_EFFECTIVE`
- Approved authority source commit: `82f8f5193b658310e73e9f2fb4abf13ebb954076`
- Guide approved by: user (explicit human approval in Codex task)
- Guide approved at: `2026-08-17T13:16:16Z`
- Manual-acceptance state: Approved and locked; implementation, manual execution and Evaluation require separate authorization.
...
is not authority. A human identity string is only a syntax-level field: `"NhiBuaa"` is a
non-placeholder identity fixture, not production authorization. Production authority passes only
when the exact approved payload, attestation, sealed binding and closure identities below match.
...
### Authority-chain regression readback

- The old `YOUR_IDENTITY` chain at attestation commit
  `b8d4b3bf417fd07d48e8b9019c0275b6b4dbe9da` is integrity-valid but authority-invalid with
  `AUTHORITY_VALIDATION_FAILURE / HUMAN_IDENTITY_PLACEHOLDER`; its old seal and closure remain
  audit evidence only.
- The exact new sealed chain pinned above is the only positive production authority case and must
  return `APPROVED_EFFECTIVE` with closure `status: PASS`.
- An alternate derived chain with `reviewer_id`/`approved_by = "NhiBuaa"` but any changed
  approval payload, attestation Git identity or sealed binding must return
  `AUTHORITY_VALIDATION_FAILURE`. The string is not an allowlist or a substitute for the exact
  approved chain.
...
- Execution gate: the claim-rule authority is `APPROVED_EFFECTIVE` and this v6 guide is approved and locked. Implementation, manual execution and Evaluation remain separate transitions requiring explicit authorization; do not start them in this lock operation.
...
- Steps:
  ...
  4. Create disposable negative fixtures that mutate each authority identity, projection policy
     ...
     Also submit an alternate chain whose `reviewer_id`/`approved_by` remain `"NhiBuaa"` while
     one approval payload, attestation Git identity or sealed binding differs. Submit each fixture
     to canonical production selection.
...
- Expected results:
  ...
  - An integrity-valid chain whose reviewer or approver identity is a placeholder returns
    ...
    `NO_CLAIM`.
  - The low-level identity-validation seam accepts `"NhiBuaa"` as a non-placeholder syntax fixture,
    but an alternate or unapproved chain with the same string returns
    `AUTHORITY_VALIDATION_FAILURE`. Only the exact approved sealed `NhiBuaa` chain passes production
...
- Steps:
  ...
  2. Confirm tests include absent/empty/incomplete/extra/non-boolean/false guardrails; malformed/mismatched provenance; category applicability and denominator reconciliation; hidden-evidence semantic negative; every taxonomy fixture and stage-invalid finding; missing/stale/malformed/digest-mismatched/non-effective claim-rule authority; placeholder human identities including `YOUR_IDENTITY` and `YOUR_REAL_IDENTITY`; a low-level
     non-placeholder syntax fixture `"NhiBuaa"`; the exact approved sealed `NhiBuaa` chain; an
     alternate/unapproved `NhiBuaa` chain that must fail authority validation; policy-field/version mutations; caller-override rejection; `OBSERVATION_FAILURE -> NO_CLAIM`; valid pair with no qualifying delta -> `NO_CLAIM`; and a qualifying selected claim retaining `claim_rule_version` and `claim_rule_digest`.
  3. Record the focused command and complete output without running production Q&A or locked manual cases.
- Expected results:
  - All focused tests pass and demonstrate both accepted and rejected fixtures, including
    placeholder human identity -> `AUTHORITY_VALIDATION_FAILURE`, alternate/unapproved `NhiBuaa`
    chain -> `AUTHORITY_VALIDATION_FAILURE`, exact approved sealed chain -> `APPROVED_EFFECTIVE`,
    both explicit no-claim regressions and the qualifying path.
...
This v6 guide is approved and locked. Its test semantics are immutable; any later semantic change requires `m3-remediation-issue-63-v7`. v5, v4, v3, v2 and v1 remain unchanged and must not be executed. Store future run observations separately as JSONL Evaluation records.
