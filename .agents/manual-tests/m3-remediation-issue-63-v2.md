...
- Supersedes: unapproved `m3-remediation-issue-63-v1`; v1 remains unchanged and must not be executed
- Guide revision: `m3-remediation-issue-63-v2`
- Approval status: pending external review and explicit human approval
- Approved by: pending
- Approved at: pending
...
- Execution gate: this v2 guide is pending approval. Do not implement or execute any case, and do not create an Evaluation record, before explicit human approval and guide lock.
...
- Steps:
  ...
  3. Correlate each observation with its public response and persisted trace, and include one applicable `EVALUATION_OBSERVATION_FAILURE` fixture in a category that otherwise has valid observations.
  4. Inspect aggregate and every category/metric section, including the observation-failure fixture, and reconcile `applicable_count`, `inapplicable_count`, `observation_failure_count`, `numerator`, `denominator` and `value`.
  5. Capture the exact semantic-scorer request input for one valid answer and repeat with a negative fixture whose hidden trace candidate would support the citation while the public excerpt does not.
- Expected results:
  ...
  - The observation-failure fixture is retained in the report/audit, is not converted to zero quality, is not counted as inapplicable, and contributes neither numerator nor denominator.
...
- Evidence to capture:
  ...
  - Aggregate and every category reconciliation table, including the failure fixture and its audit record.
  - The exact semantic-scorer input payload and the negative hidden-evidence fixture result.
...
### TC-04: Selected-improvement record retains regressions and maps invalid evidence to no-claim

- Purpose: Verify a genuinely qualifying pair produces the complete selected-improvement artifact while every invalid or non-qualifying path remains conservative.
- Steps:
  1. Use a valid baseline/hybrid pair with complete provenance and all closed guardrails true, with the hybrid satisfying the locked improvement policy.
  2. Run paired comparison and improvement selection and inspect the selected-improvement record.
  3. Use a second valid pair with no qualifying metric delta, leaving all provenance and guardrails valid.
  4. Use a pair containing an `EVALUATION_OBSERVATION_FAILURE` observation, and a separate pair with malformed/incomplete guardrails or provenance.
- Expected results:
  - The qualifying record is selected only after all metric and guardrail policy checks pass and retains deterministic metric deltas, guardrail values, latency trade-offs and a deterministic `remaining_regressions` section, including an empty section when none remain.
  - A valid pair with no qualifying delta returns `NO_CLAIM` with `selected_improvement=null` and an explicit non-qualifying reason.
  - `OBSERVATION_FAILURE` maps to `NO_CLAIM` with `selected_improvement=null`; it is never treated as a zero-quality success or upgraded to a claim.
  - Malformed guardrails/provenance return their explicit fail-closed no-claim reason and cannot select an improvement.
- Evidence to capture:
  - `improvement.json` for the qualifying pair and all no-claim variants, including selected configuration IDs, deltas, guardrails, trade-offs, regressions and reasons.
...
- Purpose: Verify executable focused coverage for malformed reports, closed guardrails/provenance, applicability, observation failure, stage-invalid findings, no-claim and qualifying-claim paths while keeping the full-suite limitation explicit.
- Steps:
...
This v2 guide is not locked. It becomes immutable only after external review and explicit human approval. Any later semantic change requires `m3-remediation-issue-63-v3`; v1 remains unchanged and must not be executed. Store future run observations separately as JSONL Evaluation records.
