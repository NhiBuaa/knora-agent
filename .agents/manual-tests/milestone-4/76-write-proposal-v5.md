# Manual Test Guide: M4.2 write proposal v5
...
- Guide revision: `m4-76-write-proposal-v5`
- Supersedes: unapproved v4 after external `REQUEST_CHANGES`
- Test-case source: `.agents/review/m4-issue-76-test-cases-v5.json`
- Prior review evidence: `.agents/review/m4-issue-76-guide-external-review-v4.json`
- External review evidence: pending `.agents/review/m4-issue-76-guide-external-review-v5.json`
- Human approval evidence: pending `.agents/review/m4-issue-76-guide-approval-v5.json`
...
- Install an explicit proposal-lookup sentinel at the application/store lookup seam. It records a
  per-request count and ordered event sequence without performing a hidden lookup. TC-03 fails if an
  unauthenticated or wrong-Workspace call records any lookup, or if an authorized lookup is not
  ordered after successful Workspace authorization.
- Core #76 workflow and unit tests depend only on `CapabilityResolver` and
  `ProposalTargetVerifier` protocols/fakes. Integration composition/tests may and must consume the
  accepted #75 concrete `CapabilityRegistry` and `ReferenceVerifier`; no provider write, #77
  execution or #78 reconciliation behavior enters this guide.
- For TC-07, “restart” means recreate the PostgreSQL-backed store/application composition with new
  application objects while retaining the same durable database rows. Each condition has its own
  before/after-restart observation pair.
- Record exact commands/exit codes, sanitized responses, field-by-field PostgreSQL projections,
  audit/decision counts, authorization/lookup order, sentinel identities/counts, subject SHA and
  clean status.
- The candidate Evaluation must contain a unique run ID, this guide revision/digest, exact subject
  ...
  `human_approval` status. Append through `record_evaluation.py`; never rewrite v3/v4 history.
...
- Steps:
  ...
  2. Mutate original containers and attempt top-level/nested mutation through every retained policy
     view before and after proposal creation.
...
- Expected results:
  - Construction takes one deep immutable canonical snapshot and retains no caller-owned mutable
    container; direct/nested mutation is rejected or cannot affect the value.
  - Snapshot, digest and persisted semantics remain exactly consistent under every attempt.
  - Policy-selected lifetime is present in the attested snapshot; proposal creation reads one
    internally consistent policy value.
...
- Expected results:
  ...
  - No raw provider/key/MAC/internal reference data appears in response or evidence.
- Evidence to capture: complete named matrix, safe envelopes, digest recomputation and zero
  row/write counters.

### M4-76-TC-03: Workspace authorization observably precedes proposal lookup

- Purpose: prove ordering directly rather than infer it from HTTP status and prevent cross-Workspace
  existence leakage or mutation.
- Steps:
  1. Reset the proposal-lookup sentinel; exercise create/read/approve/reject without authentication
     and capture the authorization/lookup event sequence and count.
  2. Reset it; repeat under the wrong Workspace and capture the same observations.
  3. Reset it; as an authorized principal read and decide an absent proposal, capturing order/count.
- Expected results:
  - Missing auth is `401/UNAUTHENTICATED`, and every such request records zero proposal lookups.
  - Wrong-Workspace calls are `403/WORKSPACE_ACCESS_DENIED` and record zero proposal lookups.
  - Authorized absence is `404/TOOL_PROPOSAL_NOT_FOUND`; its trace records successful Workspace
    authorization before exactly one scoped proposal lookup.
  - Denied calls add no decision/audit row and leave the provider-write sentinel at zero.
- Evidence to capture: route/status/error matrix, per-call lookup count, ordered
  authentication/authorization/lookup trace, row counts and write counter.
...
- Purpose: prove the durable store preserves complete reviewed intent and never inherits a decision
  across material changes.
- Steps:
  ...
  3. Through trusted composition, independently change policy semantics, title and target reference
     and create replacements using only public request fields.
- Expected results:
  ...
  - Each material change creates new proposal/logical identities with no inherited approval;
    provider sentinel remains zero.
...
- Purpose: prove expiry is selected by attested policy through production composition and prove each
  denial/stale/expiry projection survives PostgreSQL-backed restart without #77/#78 behavior.
- Steps:
  1. Use accepted #75 registry/reference composition with a trusted non-default lifetime and clock;
     create/approve the proposal and prove the expiry calculation.
  2. Temporarily deny current execution authority. Record projection A, recreate the PostgreSQL-
     backed composition, reload the same proposal and record projection B.
  3. Restore exact compatibility. Independently create each capability, binding and policy mismatch;
     for each mismatch record before-restart A, recreate composition, reload the same proposal and
     record after-restart B. Restore exact compatibility between mismatch cases.
  4. Restore exact compatibility, advance beyond expiry, record A, recreate composition, reload the
     same proposal and record B.
- Expected results:
  - `expires_at` equals trusted creation time plus the policy-selected non-default lifetime; the
    lifetime is inside the exact policy snapshot/digest and no independent one-hour fallback exists.
  - Every A/B pair is identical: temporary denial remains `approved`, `stale=false`,
    `executable=false` with `execution_not_authorized`; each material mismatch keeps the decision
    but is stale/non-executable with its exact typed reason; expiry is non-executable with its typed
    expired reason.
  - Proposal/decision/audit values and durable identity are unchanged across restarts; execution,
    reconciliation and provider-write counters remain zero.
- Evidence to capture: composition dependency trace, policy snapshot/digest/expiry calculation,
  condition-by-condition A/B restart matrix, database/projection identity, unchanged decision/audit
  values and zero counters.

### M4-76-TC-08: One canonical JSON authority, reconciled #75 seam and governed verification

- Purpose: eliminate duplicate canonical semantics and prove the accepted integration boundary with
  an observable no-write oracle.
- Steps:
  1. Inspect imports/diff and run canonical-json-v1 conformance tests for capability, reference,
  ...
  2. Prove core #76 workflow/tests use narrow protocols/fakes while integration composition/tests
     use accepted #75 `CapabilityRegistry` and `ReferenceVerifier` without changing the protocols.
  3. With explicit counting write and proposal-lookup sentinels installed, recreate/migrate the
     database, run focused M4.2 tests, full pytest, Ruff and Compose; recreate/migrate once more and
     inspect clean status.
- Expected results:
  - `knora.tools.contracts` is the single canonical-json-v1 implementation; no duplicate
    `proposal_contracts` authority remains and conformance output is identical across consumers.
...
- Evidence to capture: import/dependency inventory, conformance results, exact invocations/exits/test
  totals/migration head, lookup/write sentinel identities/counts, diff and clean status.

Observations belong to
`.agents/manual-tests/milestone-4/76-write-proposal-v5.evaluations.jsonl`. Append a candidate record
