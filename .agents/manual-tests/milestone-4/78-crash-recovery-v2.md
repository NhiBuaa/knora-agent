# Manual Test Guide: M4.4 crash recovery and provider reconciliation v2
...
- Guide revision: `m4-78-crash-recovery-v2`
- Test-case source: `.agents/review/m4-issue-78-test-cases-v2.json`
- Prior guide review: `.agents/review/m4-issue-78-guide-external-review-v1.json`,
  `REQUEST_CHANGES` with two Major security-oracle findings
- External guide review evidence: pending `.agents/review/m4-issue-78-guide-external-review-v2.json`
- Human approval evidence: pending `.agents/review/m4-issue-78-guide-approval-v2.json`
- Revision scope: v2 changes only the successful observation-authorization ordering oracle and the
  observation-snapshot/write-authority separation oracle requested by guide review v1. All other
  v1 cases, result matrices and release boundaries remain unchanged.
...
- Install independent ordered/counting sentinels for authentication, path-Workspace binding,
  ...
  provider-ledger rows and external ticket effects. A sentinel for one authority cannot stand in for
  another.
...
- Steps:
  ...
  4. After each observation returns not-found, attempt every retry/write seam: retry decision,
     admission creation or reuse, dispatch and gateway `create_ticket`. Capture target lookup,
     provider-ledger and provider-effect counts. Prove no snapshot field/provenance is accepted as
     write-authorizer input or result.
...
- Expected results:
  ...
  - In every valid-snapshot/current-write-denied row, observation routing remains usable and may
    return not-found, but retry is denied and new/reused admission, dispatch, `create_ticket`, target
    lookup, provider-ledger mutation and provider-effect counts are each exactly zero.
...
- Evidence to capture: expiry/revocation terminal matrix; observation-denial order/zero counters;
  valid-snapshot/current-write-denial differential matrix for every retry/write seam; snapshot-field
  exclusion from write-authorizer inputs; admission/dispatch/create-ticket/lookup/ledger/effect zero
  counts; proposal/approval before-after state; mismatch matrix; sanitized routing-fault results.
