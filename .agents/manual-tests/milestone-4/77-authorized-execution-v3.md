# Manual Test Guide: M4.3 authorized execution v3
...
- Guide revision: `m4-77-authorized-execution-v3`
- Test-case source: `.agents/review/m4-issue-77-test-cases-v3.json`
- Prior guide review: `.agents/review/m4-issue-77-guide-external-review-v2.json`,
  `REQUEST_CHANGES` with 4 Major findings and no Critical or Minor findings; it confirmed all three
  guide-v1 findings closed
- External guide review evidence: pending `.agents/review/m4-issue-77-guide-external-review-v3.json`
- Human approval evidence: pending `.agents/review/m4-issue-77-guide-approval-v3.json`
...
- TC-03 loads its expected denial matrix from a test fixture that imports no production admission
  result type or mapper. TC-10 likewise loads the immutable result matrix below from a release-
  harness fixture that imports or calls no production result class, enum, serializer or HTTP mapper.
...
## Immutable M4.3 public-result matrix

This table exhausts every post-acquisition or repeated-execute result that Issue #77 may publish.
Pre-acquisition authentication, authorization, stale, expiry and validation failures remain the
existing closed `{"error":{"code":"..."}}` envelope tested by TC-01 and do not use this matrix.
Every matrix row has exactly four common fields plus exactly the named variant field; all other
fields at every depth are forbidden.
...
The common fields are exactly `proposal_id`, `logical_execution_id`, `lifecycle` and
`outcome_type`. Both IDs are canonical lowercase RFC 4122 UUID strings read from the durable
proposal/execution identity. The HTTP body is field-for-field equal to the application projection;
the status code is transport metadata and does not appear in the body. The matrix is immutable for
this guide revision.
...
- Purpose: close every acquisition-to-admission TOCTOU window with a deterministic result for every
  non-key and key-lifecycle row.
- Steps:
  1. Acquire and block immediately before private admission authorization.
  2. Run separate rows for Workspace, resource and execution authority denial; capability identity,
     version and digest mismatch; binding identity, version and digest mismatch; policy identity,
     version and digest mismatch; canonical-byte, MAC and protected-claim corruption; and trusted-
     store delete, replacement and claims mismatch.
  3. Run the complete key-lifecycle matrix after locking: active exact key, retiring exact key,
  ...
  4. Run an exact compatible unmutated control and compare every row to the independently loaded
     expected result before inspecting admission/audit and gateway/provider sentinels.
- Expected results:
  - The exact non-key typed result matrix is:
    `Workspace authority -> WORKSPACE_ACCESS_DENIED`;
    `resource authority -> TOOL_RESOURCE_ACCESS_DENIED`;
    `execution authority -> TOOL_EXECUTION_NOT_AUTHORIZED`;
    capability identity/version/digest -> matching
    `capability_identity_mismatch|capability_version_mismatch|capability_digest_mismatch`;
    binding identity/version/digest -> matching
    `binding_identity_mismatch|binding_version_mismatch|binding_digest_mismatch`;
    policy identity/version/digest -> matching
    `policy_identity_mismatch|policy_version_mismatch|policy_digest_mismatch`;
    non-canonical bytes -> `INVALID_TOOL_RESOURCE_REFERENCE`; and MAC, protected-claim, trusted-
    store delete/replace/mismatch -> `TOOL_RESOURCE_ACCESS_DENIED`.
  - Every denial happens after post-lock reload and persists zero admission rows, zero admission-
    audit rows and makes zero gateway/provider calls for that row's named sentinels.
  - The compatible control, active exact and retiring exact rows admit exactly once when every other
    fact is compatible; admission binds the post-lock key ID/version and epoch and makes exactly one
    provider call.
  - Revoked, unknown and unavailable rows return `TOOL_RESOURCE_ACCESS_DENIED`, persist zero
    admission/audit and make zero gateway/provider calls.
...
- Evidence to capture: independently sourced complete row/result matrix, compatible control,
  post-lock key ID/version and epoch, lock/reload order, per-row admission/audit/gateway/provider
  counts and sanitized-evidence scan.
...
### M4-77-TC-10: Generation-1 fencing and complete sanitized public-result matrix

- Purpose: prevent expired-owner finalization, implementation-derived result expectations and
...
- Steps:
  1. Start with provider-only success and each closed rejection, no PostgreSQL observation and
     ...
     and owner-correlated PostgreSQL write trace.
  ...
  3. Repeat before expiry for success and all three closed rejections; exercise indeterminate,
     current execution-in-progress, provider-idempotency conflict and fenced application/HTTP rows.
  4. Load the expected immutable matrix without importing/calling production result classes, enums,
     serializers or HTTP mappers. For every row compare exact `outcome_type`, `lifecycle`, HTTP
     status, complete key set and allowed value domain; inject every forbidden field recursively.
- Expected results:
  - Each expired operation uses fresh post-lock PostgreSQL time and returns `ExecutionFenced`.
    ...
    by the expired owner; ordered IDs/counts and stable row-set digest remain exact. Provider truth
    remains readable after restart.
  - Every application result and decoded HTTP body is field-for-field equal to one and only one row
    of the immutable matrix above, including the exact lifecycle/status and variant value.
  - Common fields are exactly `proposal_id`, `logical_execution_id`, `lifecycle`, `outcome_type`;
    both IDs are canonical lowercase UUIDs and exactly one approved variant field is present.
...
- Evidence to capture: post-expiry execution-row snapshot, ordered observation/audit identities and
  ...
  record, independent matrix source/digest, complete application/HTTP row comparison and forbidden-
  field failures.
...
`.agents/manual-tests/milestone-4/77-authorized-execution-v3.evaluations.jsonl`. Append a candidate
