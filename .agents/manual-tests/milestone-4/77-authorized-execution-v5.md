# Manual Test Guide: M4.3 authorized execution v5
...
- Guide revision: `m4-77-authorized-execution-v5`
- Test-case source: `.agents/review/m4-issue-77-test-cases-v5.json`
- Prior guide review: `.agents/review/m4-issue-77-guide-external-review-v4.json`,
  `REQUEST_CHANGES` with 2 Major findings and no Critical or Minor findings: TC-10 lacked a
  discriminating post-lock PostgreSQL-time oracle for generation-1 observation/finalization, and
  TC-03/TC-10 did not independently bind denial-row proposal/execution identity values
- External guide review evidence: pending `.agents/review/m4-issue-77-guide-external-review-v5.json`
- Human approval evidence: pending `.agents/review/m4-issue-77-guide-approval-v5.json`
...
- Steps:
  ...
  4. Exercise every TC-03 post-acquisition denial row through application and HTTP seams. Before
...
- Evidence to capture: separate observation/finalization clock tuples containing lease deadline,
  ...
  counts; stable PostgreSQL row-set digests; provider restart record; independent matrix source/
...
`.agents/manual-tests/milestone-4/77-authorized-execution-v5.evaluations.jsonl`. Append a candidate
