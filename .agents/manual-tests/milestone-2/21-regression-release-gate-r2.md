...
- A generated baseline manifest that contains each collected pytest node ID, outcome and approved disposition.
...
- Steps:
  1. Record the candidate SHA and clean-worktree status.
  2. Generate the candidate test manifest from pytest collection and execution results.
  3. Compare the candidate manifest with the approved baseline allowlist or a machine-generated baseline manifest from the pinned predecessor candidate.
  4. Run Ruff, Compose configuration verification, and existing synchronous ingestion, question/citation/refusal, authentication and evaluation suites.
- Expected results:
  - Every required command passes.
  - The manifest diff has no added, removed or outcome-changed node ID unless the exact difference is in the approved baseline allowlist.
  - The allowlist records each permitted node ID, exact disposition, reason, authority and expiry. A total pass, skip or xfail count does not satisfy this oracle.
  - Markdown/plain-text ingestion and existing cited-answer/refusal contracts remain unchanged.
- Evidence: candidate SHA, baseline-manifest SHA, machine-readable diff, approved allowlist, commands/selectors, per-outcome node IDs, lint/Compose outputs and safe HTTP samples.
