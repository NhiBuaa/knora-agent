...
- Guide revision: `m2-issue-18-r8`
- Supersedes: draft `m2-issue-18-r7` at
  `.agents/manual-tests/milestone-2/18-pdf-derivation-activation-r7.md`; r1 through r7 remain
  unchanged.
- Approved by: Nhi (explicit human approval in Codex task)
- Approved at: 2026-08-09T22:37:58+07:00
- Manual-acceptance state: Approved for implementation; execution remains pending implementation.
...
- Instrumentation: provider/extractor/ObjectStore call counters; parent raw-source-retention probe
  with source-derived byte tagging/reference accounting and `peak_live_parent_raw_source_bytes`;
  injected monotonic heartbeat schedule; PostgreSQL fresh-time/row-lock trace;
  transition-operation-ID replay trace; and a correlated transaction probe recording transaction
  ID, connection ID, owner/span kind and begin/end for claim/recovery, heartbeat,
  handler/configuration/persistence lookup and finalization/activation operations.
...
| Original-object checksum and ObjectStore metadata are verified; reads stream | TC-07A1–TC-07A5; TC-07B steps 1–3 | Each required claimed-vs-observed metadata component (Workspace, opaque key, raw SHA-256, byte size, media type) is independently rejected before open/read/extraction/provider; TC-07B separately proves no whole-object read or complete logical parent retention | Per-field injected mismatch/category, zero open/extractor/provider counts, zero derivation/activation delta, unchanged active pointer; parent raw-source-retention/peak-live-byte trace |
...
| Embedding calls occur outside DB transactions and validate count/dimension/provider/model/configuration | TC-01A step 2; TC-01C steps 1–3; TC-03B steps 1–3 | No handler/configuration/persistence or finalization/activation transaction encloses remote work; independent short heartbeat transactions may overlap | Correlated transaction ownership trace, finalization lock trace, provider trace, vector validation projection |
| Retry taxonomy covers the Issue #18 PDF-handler categories; deterministic failures are terminal | Executable handler taxonomy: TC-03A–TC-03E and TC-03G–TC-03L. `DATABASE_TRANSIENT`: TC-03F is not applicable. Coordinator semantics: TC-05B, TC-05C and TC-06C. | Handler facts map through the executable TC-03 cases; coordinator PostgreSQL failures retain Issue #17 reconciliation/indeterminate semantics rather than entering handler taxonomy. | Handler cause/policy/attempt evidence for TC-03; same-operation reconciliation evidence from TC-05B; definite pre-commit persistence-failure evidence from TC-05C; lease-expiry recovery evidence from TC-06C. |
...
#### TC-01C: Prove remote work is not enclosed by a handler/persistence transaction

- Injection: a correlated transaction probe records transaction ID, connection ID, operation/span
  kind, owner and begin/end for every attempt connection. Its kinds distinguish claim/recovery,
  heartbeat, handler/configuration/persistence lookup, and finalization/activation. Extractor,
  chunker and provider spies record begin/end; the provider returns the fixed valid batch while a
  controlled short heartbeat transaction begins and commits independently.
- Steps:
  1. Run the profile-A fixture through one worker iteration.
  2. Inspect every transaction whose lifetime intersects extraction/chunking or
     `provider.begin -> provider.end`.
  3. Inspect the ordered probe trace after success.
- Expected results:
  - No handler/configuration/persistence lookup transaction begins before extraction/chunking or
    provider activity and remains open through it. The provider does not execute within, inherit,
    or become enclosed by any such transaction context.
  - No finalization/activation transaction begins before valid provider output exists. A short,
    independent heartbeat transaction may begin and commit while remote work runs and is not a
    failure.
  - The probe fails a long-held handler/configuration/persistence or finalization transaction around
    remote work, while passing the controlled concurrent heartbeat.
  ...
  - Finalization runs only after the valid provider output exists and produces TC-01A's atomic
    outcome.
- Evidence to capture:
  - Correlated transaction records and ownership classification, extraction/chunking/provider
    boundaries, concurrent heartbeat begin/end, finalization begin/end and lock events, plus final
    lifecycle/derivation projection.
...
Every executable handler subcase starts from attempt 1 of one profile-A job. For `ScheduleRetry`,
expected policy is V1 `[0, 5 seconds]` full jitter recorded once; a separately controlled run
advances to the exact persisted `next_attempt_at` and proves claim of attempt 2. For
`FailTerminal`, expected attempt count remains one and there is no provider/extractor call after the
named failure point.
...
| TC-03F | Not applicable: the Issue #18 PDF handler has no handler-owned database dependency | `DATABASE_TRANSIENT` is unreachable for this concrete handler | No handler injection; coordination PostgreSQL failures use Issue #17 persistence/reconciliation or indeterminate-infrastructure semantics | No handler/provider rerun or invented lifecycle result from coordination failure |
...
TC-03F is deliberately not satisfied by coordination-store deadlock, serialization, connectivity,
network or ambiguous-commit failure: updated Issue #18 and Issue #17 require those failures to use
persistence/reconciliation or indeterminate-infrastructure semantics, never
`DATABASE_TRANSIENT`. The Issue #18 PDF handler introduces no handler-owned database dependency;
`DATABASE_TRANSIENT` remains available to future WorkHandlers that legitimately observe one. Shared
allowlisted public codes are valid for transient categories unless an approved contract names a
variant-specific code.

Evidence for every executable handler subcase is the injected sentinel, handler kind, canonical
cause, policy result, safe code, full attempt-history retry/terminal fields, call counts, and zero
derivation/active-pointer delta.

TC-03F evidence is the authoritative Issue #18 criterion, the Issue #17 cause/persistence
contract, proof that the PDF handler has no handler-owned database dependency, and absence of a
handler injection. Coordination failure/reconciliation evidence comes from TC-05B's same-operation
read-back, TC-05C's definite pre-commit persistence failure and TC-06C's `LEASE_EXPIRED` recovery;
it is never an invented handler outcome.
...
#### TC-07A: Independently verify every required Original Source Object metadata component

The approved Issue #18 design requires `ObjectStore.head` to exactly match the claimed Workspace,
opaque object key, SHA-256, byte size and media type. These five fields are ObjectStore metadata in
the Current World Model and Architecture Standard; no other technical field is added to this matrix.
Each subcase starts with a Document that has a prior active Embedding Set. The durable job expects
metadata tuple `(workspace_id=W1, object_key=K1, sha256=H1, byte_size=S1, media_type=M1)`.

| Subcase | Fixed `ObjectStore.head` observation | Expected result | Evidence to capture |
| --- | --- | --- | --- |
| TC-07A1 | `(W2, K1, H1, S1, M1)` where `W2 != W1` | Workspace mismatch is detected before `open_read`, extraction or embedding | Internal category `workspace_mismatch`; safe expected/observed identity evidence; head/open/extractor/provider counts |
| TC-07A2 | `(W1, K2, H1, S1, M1)` where `K2 != K1` | Opaque-key mismatch is detected before `open_read`, extraction or embedding | Internal category `object_key_mismatch`; safe key-comparison digest (not key value); head/open/extractor/provider counts |
| TC-07A3 | `(W1, K1, H2, S1, M1)` where `H2 != H1` | Raw-SHA-256 mismatch is detected before `open_read`, extraction or embedding | Internal category `raw_sha256_mismatch`; safe checksum digests; head/open/extractor/provider counts |
| TC-07A4 | `(W1, K1, H1, S2, M1)` where `S2 != S1` | Byte-size mismatch is detected before `open_read`, extraction or embedding | Internal category `byte_size_mismatch`; expected/observed size class; head/open/extractor/provider counts |
| TC-07A5 | `(W1, K1, H1, S1, M2)` where `M2 != M1` | Media-type mismatch is detected before `open_read`, extraction or embedding | Internal category `media_type_mismatch`; safe media-type category; head/open/extractor/provider counts |

For every subcase, `open_read`, extractor and provider call counts are zero; the handler takes the
canonical terminal input path (`INVALID_INPUT` -> `FailTerminal`,
`failure_reason=terminal_input`); no derivation/activation mutation occurs; and prior active
knowledge remains active and retrievable. The public response uses the one approved allowlisted
source-integrity/input-metadata code where the contract permits sharing; the distinct internal
mismatch category is retained only in safe test evidence, never exposed as raw object metadata.

#### TC-07B: Worker consumes the source incrementally without whole-object materialization

- Injection: `ObjectStore.open_read` returns an instrumented incremental source whose derived
  chunks carry source-byte tags. A parent raw-source-retention probe uses tag/reference accounting
  across buffers, containers, wrappers and equivalent representations to record
  `peak_live_parent_raw_source_bytes`. It fails if a whole-object read occurs or if live parent
  references simultaneously retain all logical raw-source bytes. The isolated extractor test
  adapter may receive a bounded wrapper, pipe, file descriptor or bounded spool-to-disk; it returns
  the fixed projection after consuming the complete source through that approved bounded transport.
- Steps:
  1. Run the valid fixture handler to extraction completion.
  2. Compare `peak_live_parent_raw_source_bytes` with the fixed fixture's `raw_source_size` and
     inspect source-consumption/whole-object-read traces before provider/finalization.
- Expected results:
  - No worker/application whole-object read API is called, and the application does not materialize
    the full raw PDF in memory. The deterministic retention predicate proves
    `peak_live_parent_raw_source_bytes < raw_source_size`, regardless of whether retained bytes
    would otherwise be contiguous, split across chunks, stored in a container or wrapped.
  - The retention probe rejects all three invalid shapes: whole-object read, bounded reads followed
    by a joined full source value, and bounded reads whose every source-derived chunk remains live
    in the parent application without joining.
  - Source consumption is incremental. Its bounded transport remains subject to the approved 25 MiB
    raw-object limit; isolated extractor RSS is verified separately against its 256 MiB budget and
    is not used as the no-materialization oracle.
...
- Evidence to capture:
  - Whole-object-read count zero, incremental-consumption trace, `raw_source_size`,
    `peak_live_parent_raw_source_bytes`, tagged-reference accounting report, bounded transport
    classification, extractor/provider counts and lifecycle result.
...
None. GitHub Issue #18 now authoritatively states that `DATABASE_TRANSIENT` is unreachable for this
PDF handler and that coordination PostgreSQL failures retain the Issue #17 persistence,
reconciliation or indeterminate-infrastructure semantics.
...
`m2-issue-18-r8` after approval. Store future execution observations separately as JSONL
Evaluation records.
