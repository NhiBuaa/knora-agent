...
- Status: Draft — revision `m2-issue-18-r2` awaits explicit human approval. Do not implement or
  execute this guide.
...
- Guide revision: `m2-issue-18-r2`
- Supersedes: draft `m2-issue-18-r1` at
  `.agents/manual-tests/milestone-2/18-pdf-derivation-activation.md`; r1 remains unchanged.
- Approved by: Pending
- Approved at: Pending
...
| Embedding calls occur outside DB transactions and validate count/dimension/provider/model/configuration | TC-01A step 2; TC-01C steps 1–3; TC-03B steps 1–3 | Provider completes before final transaction/activation lock; only exact vector identity persists | Transaction/lock probe, provider trace, vector validation projection |
| Retry taxonomy covers approved transient categories; deterministic failures are terminal | TC-03A–TC-03F and TC-03G–TC-03K | Each fixed injection maps to its named failure kind/cause, policy disposition and safe result | Handler fact, canonical cause, attempt history, policy metadata, call counts |
...
#### TC-01C: Prove provider work is outside final persistence transactions

- Injection: transaction/lock probe marks finalization transaction begin/end and each
  job/attempt/document/activation lock; provider spy marks begin/end. The provider blocks until the
  probe confirms no finalization transaction is open, then returns the fixed valid batch.
- Steps:
  1. Run the profile-A fixture through one worker iteration.
  2. Release the provider only after the probe asserts no finalization transaction or activation
     lock is active.
  3. Inspect the ordered probe trace after success.
- Expected results:
  - Extraction and provider completion precede finalization transaction begin and activation-row
    locking. No database transaction is held across remote provider work.
  - Finalization runs only after the valid provider output exists and produces TC-01A's atomic
    outcome.
- Evidence to capture:
  - Monotonic ordered provider/transaction/lock events and final lifecycle/derivation projection.
...
- Expected results:
  ...
  - N is terminal `failed` with one counted closed attempt, no retry, `failure_reason=terminal_input`
    and the approved allowlisted textless-PDF safe code.
- Evidence to capture:
...
#### TC-02B: Invalid pinned configuration does not fall back to profile B

- Injection: enqueue under profile A, then mark A's immutable extraction/profile record invalid
  while profile B is valid and selected as mutable current. The handler receives A and emits
  `HandlerFailureKindV1.CONFIGURATION_INVALID` before object read/extraction/provider work.
- Steps:
  1. Switch controlled current selection to B after enqueueing A.
  2. Run the A job once.
- Expected results:
  ...
  - The exact public safe code is the pinned-config code named by the implementation contract;
    until that code is named, this case remains an approval blocker listed below rather than an
    operator-chosen value.
- Evidence to capture:
  - Handler failure fact/cause/policy/attempt history, profile traces, zero call counts and zero
    derivation/pointer delta.
...
| --- | --- | --- | --- | --- |
| TC-03A | Provider raises fixed timeout classification after extractor succeeds | `PROVIDER_TRANSIENT` -> `PROVIDER_TRANSIENT` | `ScheduleRetry`; exact provider-timeout allowlisted code | extractor 1, provider 1; no derivation/activation; closed retry attempt with V1 audit |
| TC-03B | Provider returns fixed HTTP 429 classification after extractor succeeds | `PROVIDER_TRANSIENT` -> `PROVIDER_TRANSIENT` | `ScheduleRetry`; exact provider-throttled allowlisted code | extractor 1, provider 1; same retry-only delta |
| TC-03C | Provider returns fixed HTTP 503 classification after extractor succeeds | `PROVIDER_TRANSIENT` -> `PROVIDER_TRANSIENT` | `ScheduleRetry`; exact provider-5xx allowlisted code | extractor 1, provider 1; same retry-only delta |
| TC-03D | `ObjectStore.open_read` raises the approved transient-storage sentinel before extractor starts | `STORAGE_TRANSIENT` -> `STORAGE_TRANSIENT` | `ScheduleRetry`; exact storage-transient allowlisted code | extractor 0, provider 0; same retry-only delta |
...
| TC-03F | Handler-owned approved transient database dependency raises its fixed sentinel before provider call | `DATABASE_TRANSIENT` -> `DATABASE_TRANSIENT` | `ScheduleRetry`; exact business-database allowlisted code | extractor/provider count fixed by the selected approved seam; no finalization mutation |
...
| TC-03I | Profile A is fixed invalid as in TC-02B | `CONFIGURATION_INVALID` -> `CONFIGURATION_INVALID` | `FailTerminal`, `failure_reason=terminal_config`, pinned-config code | extractor 0, provider 0; no derivation/activation |
| TC-03J | Provider returns exactly `chunk_count - 1` vectors | `VECTOR_MISMATCH` -> `VECTOR_MISMATCH` | `FailTerminal`, `failure_reason=terminal_config`, `EMBEDDING_DIMENSION_MISMATCH` only if that is the approved count-mismatch code; otherwise the named allowlisted vector-count code | extractor 1, provider 1; no derivation/activation |
| TC-03K | Provider returns chunk-count vectors of 1535 dimensions or a mismatched fixed provider/model | `VECTOR_MISMATCH` -> `VECTOR_MISMATCH` | `FailTerminal`, `failure_reason=terminal_config`, `EMBEDDING_DIMENSION_MISMATCH` for dimension mismatch and named allowlisted identity-mismatch code for provider/model mismatch | extractor 1, provider 1; no derivation/activation |
...
`DATABASE_TRANSIENT` business work. It is an approval blocker until an approved handler-owned
database dependency and sentinel exist. Likewise, every table cell that says “exact ...
allowlisted code” is a named-code blocker, not permission for an operator to choose a variant.
...
- Injection: two deliveries attempt to process the same durable job; only the valid current claim
  may execute. The second delivery receives the persisted claim/reconciliation result.
- Steps:
  1. Start delivery one through valid claim and complete its concrete success finalization.
  2. Deliver the same work again and reconcile against durable state.
  3. Inspect chain, activation and attempt history.
- Expected results:
  - Exactly one complete visible derivation/activation and one terminal attempt disposition exist.
  - No second provider/business execution or second activation occurs solely because delivery was
    repeated.
- Evidence to capture:
  - Claim/delivery trace, provider count, one-chain row projection, one attempt closure and active
    pointer.
...
- Injection: the exact observed lease expires; recovery receives its fixed policy decision.
- Steps:
...
- Expected results:
  ...
  - Its exact public safe code is an unresolved named-code blocker; no raw metadata is exposed.
- Evidence to capture:
...
#### TC-07B: Worker hands a bounded stream to the extractor without materializing the object

- Injection: `ObjectStore.open_read` returns `BoundedReadSpy`, which rejects `read()` with no size
  or a request above 64 KiB and records every request. The fixed extractor asserts it received that
  same stream and consumes it only by 64-KiB reads before returning its fixed projection.
- Steps:
  1. Run the valid fixture handler to extraction completion.
  2. Inspect reader/extractor ownership and read-request trace before provider/finalization.
- Expected results:
  - Application worker code does not call unbounded `read()`, does not replace the stream with a
    bytes buffer, and makes no direct full-object read. The extractor consumes the stream in bounded
    reads through the same capability.
  - The valid path then makes exactly one provider call and continues normally; the evidence
    distinguishes a streaming implementation from one that materializes the PDF in application
    memory.
- Evidence to capture:
  - Reader request-size trace, stream object identity at extractor boundary, worker direct-read
    count zero, extractor/provider counts and lifecycle result.
...
- Expected results:
  ...
  - If an approved cleanup-intent/outbox mechanism is implemented in this slice, its safe intent
    record is observed separately; otherwise the zero immediate-delete result is the complete
    Issue #18 assertion and cleanup ownership is deferred.
- Evidence to capture:
...
## Design ambiguities that block approval/execution of affected subcases

1. **Named safe codes:** the approved Issue #18/design text requires allowlisted safe codes but
   does not name codes for ObjectStore metadata mismatch, invalid pinned configuration, individual
   provider timeout/429/5xx, storage transient, extractor eviction, business database transient,
   vector-count mismatch or provider/model identity mismatch. The implementation design must name
   the exact public codes before TC-02B, TC-03 and TC-07A can be locked.
2. **Handler-owned database transient:** the approved worker design makes finalization/coordination
   store failures infrastructure or indeterminate outcomes, explicitly not business
   `DATABASE_TRANSIENT`. No independent handler-owned database dependency is currently named. The
   feature must either specify one with an exact sentinel (enabling TC-03F) or remove that claimed
   retry category from Issue #18 scope through an approved design decision.
3. **Activation constraints:** the design requires database-level same-Document/Workspace and
   completed-set enforcement, but the current table model alone has only simple foreign keys. The
   exact migration constraint/trigger names and SQLSTATE evidence are implementation details still
   to be designed; TC-08 fixes the required observable enforcement, not a guessed mechanism.
4. **Asynchronous cleanup intent:** ADR 0006 establishes that terminal `run_once` does not delete
   a Document Version-owned Original Source Object. It does not assign a concrete cleanup
   intent/outbox schema to Issue #18. TC-09 therefore proves the owned non-deletion invariant and
   records any separately approved intent only when one exists.
...
`m2-issue-18-r2` after approval. Store future execution observations separately as JSONL
Evaluation records.
