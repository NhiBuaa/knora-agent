# Issue #78 contract revision v5 — delivery-evidence provenance correction
...
- [ ] Provider outcome observation and terminal finalization remain allowed after the original
  ...
  authorization still passes. Required acceptance covers `found(succeeded)` and every approved
  `found(failed(closed_code))` while expired and while key-revoked: Knora terminalizes with the exact
  public mapping, makes zero `create_ticket`/retry calls and does not treat expiry/revocation as a
  finalization denial. A missing/mismatched snapshot or trusted-store record stays typed
...
- [ ] When provider-first observation returns a terminal outcome under a strictly expired lease,
  reconciliation does not persist under that lease. It attempts takeover with the observed
  generation; exactly one Applied winner records that same immutable terminal observation and
  finalizes under its new current owner/generation and unexpired lease. Losers return the typed
  fenced/current/terminal result, stale owners cannot record or finalize, no provider write/retry
  occurs and a terminal outcome is never converted into a retry. A second provider lookup is not
  required because the provider ledger outcome for the logical execution identity is immutable.
- [ ] A current lease returns `ExecutionNotStale`; changed generation returns `ExecutionFenced`;
  already-terminal state returns the persisted outcome. A terminal provider outcome with a current
  foreign lease returns `ExecutionInProgress`; stale owners cannot record or finalize after takeover.
...
- [ ] Crash after provider commit but before Knora persistence reconciles provider-authoritative
  success/failure without a duplicate side effect, including the stale-lease terminal sequence above.
...
- [ ] Exact tested-subject evidence binds the source base and the clean `tested_subject_commit` that
  was actually supplied to the focused/full verification commands. It proves #78 retains
  `CapabilityRegistry` as a static typed registry, keeps the generic provider-owned reference seam
  with only fake/SQLite adapters, and adds no runtime registration/discovery/plugin entry point,
  marketplace surface, vendor SDK/configuration or vendor-specific integration.
- [ ] Release-evidence publication is a separate non-self-referential provenance step. The committed
  evidence file binds `subject_sha` and `scope_manifest.candidate` to `tested_subject_commit`; its
  `evidence_publication_commit` must be a descendant of that subject and may change only
  `.agents/review/m4-issue-78-release-evidence-v1.json`. The worker result binds the final
  `ticket_commit`/delivery head separately. A later coordination-only commit does not silently
  become the tested subject, and a source/test/dependency change invalidates the evidence and
