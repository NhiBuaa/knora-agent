...
- [ ] Exact-candidate evidence binds source-base and candidate SHAs, focused registry tests and a
  changed-path/dependency audit proving #78 retains `CapabilityRegistry` as a static typed registry,
  keeps the generic provider-owned reference seam with only fake/SQLite adapters, and adds no runtime
  registration/discovery/plugin entry point, marketplace surface, vendor SDK/configuration or
