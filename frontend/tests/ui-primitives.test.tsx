...
describe("compact UI primitives", () => {
  ...
  it("offers the locked button variants with semantic signature and ghost styles", () => {
    render(
      <>
        <Button variant="signature">Review</Button>
    ...
    const css = readFileSync(
      path.resolve(__dirname, "../styles/controls.css"),
      "utf8",
    );
    expect(css).toMatch(
      /\.kn-button\[data-variant="signature"\][^{]*\{[^}]*var\(--signature\)[^}]*var\(--signature-foreground\)/s,
    );
    expect(css).toMatch(
      /\.kn-button\[data-variant="ghost"\][^{]*\{[^}]*background:\s*transparent/s,
    );
  ...
  it("preserves native child props and connects field labels, hints and errors", () => {
      ...
      <>
        <Field id="source" label="Source name" hint="Shown in citations">
          <input
            ...
            aria-describedby="source-context"
            required
          />
        </Field>
        ...
        <Field id="notes" label="Notes" error="Notes are required">
          <textarea rows={4} />
        </Field>
      </>,
    );
    ...
    expect(input).toHaveAttribute(
      "class",
      expect.stringContaining("source-control"),
    );
    expect(input).toHaveAttribute("placeholder", "Enter name");
    expect(input).toBeRequired();
    expect(input).toHaveAttribute(
      "aria-describedby",
      "source-context source-hint",
    );
    expect(screen.getByText("Shown in citations")).toHaveAttribute(
      "id",
      "source-hint",
    );
    expect(
  ...
  it("announces actionable errors and gives other notices visible text", () => {
    render(
      <>
        <Notice kind="error" title="Upload failed" role="alert">
          Try again
        </Notice>
        <Notice kind="info" title="Queued">
  ...
  it("names non-error notice kinds in visible text", () => {
    render(
      <>
        <Notice kind="warning" title="Update">
          Check the source
        </Notice>
        <Notice kind="system" title="Update">
          Source is processing
        </Notice>
        <Notice kind="info" title="Update">
          Processing starts shortly
        </Notice>
      </>,
    );
    expect(
      screen.getByText("Check the source").parentElement,
    ).toHaveTextContent("Warning");
    expect(screen.getByRole("status")).toHaveTextContent("In progress");
    expect(
      screen.getByText("Processing starts shortly").parentElement,
    ).toHaveTextContent("Information");
  ...
  it("preserves caller notice roles and leaves non-actionable errors non-assertive", () => {
    render(
      <>
        <Notice kind="error" title="Saved error">
          Review later
        </Notice>
        <Notice kind="error" title="Tracked error" role="status">
          Already recorded
        </Notice>
        <Notice kind="system" title="Background task" role="note">
          Working
        </Notice>
      </>,
    );
  ...
  it("renders a page heading and keyboard-focusable empty-state link", async () => {
    const user = userEvent.setup();
    const activated = vi.fn((event: React.MouseEvent) =>
      event.preventDefault(),
    );
    render(
      <>
        <PageHeader title="Documents" description="Workspace sources" />
        <EmptyState
          title="No documents"
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "No documents" }),
    ).toBeInTheDocument();
    await user.tab();
