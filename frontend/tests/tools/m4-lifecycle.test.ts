...
describe("ToolLifecycleDisplay", () => {
  it("renders the server-projected proposal, approval, execution, and reconciliation values", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({
      ...
      items: [{
        proposal: { proposal_id: "proposal-1", state: "approved", revision: 3 },
        approval: { decision: "approved", decided_at: "2026-09-18T10:00:00Z", actor_kind: "operator" },
        execution: {
          lifecycle: "succeeded",
          revision: 4,
          generation: 1,
          observations: [{ sequence: 1, observation_type: "execution_succeeded", failure_code: null, observed_at: "2026-09-18T10:01:00Z" }],
          failure_code: null,
          finalized_at: "2026-09-18T10:01:01Z",
        },
        reconciliation: { status: "observed", observation_type: "execution_succeeded", failure_code: null, observed_at: "2026-09-18T10:01:00Z" },
      }],
    }));
    ...
    expect(await screen.findByRole("heading", { name: /proposal-1/ })).toBeInTheDocument();
    ...
    expect(screen.getByText("observed", { selector: "dd" })).toBeInTheDocument();
  ...
  it("renders an explicit unavailable state without fabricating lifecycle values", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse({ availability: "unavailable", items: [], code: null })));
  ...
  it("renders an explicit observation failure state and server code", async () => {
    ...
    expect(screen.getByText(/TOOL_LIFECYCLE_OBSERVATION_FAILED/)).toBeInTheDocument();
    expect(screen.queryByRole("button")).not.toBeInTheDocument();
  });
});
