...
for (const name of [
  "KEYCLOAK_AUTHORIZATION_URL", "KEYCLOAK_TOKEN_URL", "KEYCLOAK_JWKS_URL",
  "KEYCLOAK_ISSUER", "KEYCLOAK_AUDIENCE", "KEYCLOAK_CLIENT_ID",
  "KEYCLOAK_REDIRECT_URI", "KNORA_API_URL", "KNORA_BACKEND_URL", "SESSION_SECRET",
]) delete process.env[name];

const { default: config, m5E2ERuntimeEnvironment, validateM5E2EEnvironment } =
...
describe("Playwright live runner", () => {
  ...
  it("rejects a non-local M5 endpoint", () => {
    expect(() => validateM5E2EEnvironment({ ...m5Environment, baseUrl: "https://example.test" }))
      .toThrow(/M5_E2E_BASE_URL/);
  });

  it("rejects conflicting ambient runtime settings", () => {
    expect(() => m5E2ERuntimeEnvironment(m5Environment, { KEYCLOAK_ISSUER: "https://example.test" }))
