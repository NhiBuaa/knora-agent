...
export function DocumentList({ workspaceId }: { workspaceId: string }) {
  ...
  const load = useCallback(async () => { const response = await browserRequest(`/v1/workspaces/${encodeURIComponent(workspaceId)}/documents`); if (!response.ok) { setError(`Unable to load documents (${response.status})`); return; } const body = await response.json() as { documents: DocumentResponse[] }; setDocuments(body.documents); }, [workspaceId]);
