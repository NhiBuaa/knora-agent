...
export function EvaluationView({ evaluation }: { evaluation: OperatorEvaluationResponse }) {
  ...
  return (
    <section aria-labelledby="evaluation-heading">
      ...
      <p data-tone={state.tone}>{state.tone === "normal" ? "Evaluation available" : "Evaluation unavailable"}</p>
      <p>Report: <code>{evaluation.report_id}</code></p>
      {state.detail && <p role="status">{state.detail}</p>}
      {state.tone === "normal" && <p>Evaluation data is available from the backend projection.</p>}
    </section>
  );
}
